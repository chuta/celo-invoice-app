-- Invoice Number Generation Using Sequence Table (FINAL CORRECTED VERSION)
-- This fixes the variable naming conflict
...
-- Step 3: Create new sequence-based generator (with fixed variable names)
...
BEGIN
  ...
  FROM invoices
  WHERE invoice_number LIKE v_current_month || '-%';
  
  -- Initialize the sequence table
  INSERT INTO invoice_sequences (year_month, last_sequence)
  VALUES (v_current_month, v_max_seq)
  ON CONFLICT (year_month) 
  DO UPDATE SET last_sequence = GREATEST(invoice_sequences.last_sequence, v_max_seq);
  
  RAISE NOTICE 'Initialized sequence for % with value %', v_current_month, v_max_seq;
END $$;
...
DO $$
BEGIN
  RAISE NOTICE '✅ Invoice number generation fixed successfully!';
  RAISE NOTICE 'You can now create invoices without duplicate errors.';
END $$;
